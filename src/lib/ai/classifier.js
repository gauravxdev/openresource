 ...
 */
function classifyByStructure(fileList) {
    if (!fileList || fileList.length === 0)
        return null;
    const fileNames = new Set(fileList.filter((f) => f.type === "file").map((f) => f.name.toLowerCase()));
    const dirNames = new Set(fileList.filter((f) => f.type === "dir").map((f) => f.name.toLowerCase()));
    // Check in priority order: monorepo > application > tool > template
    const checkOrder = ["monorepo", "application", "tool", "template"];
 ...
 */
function classifyByKeywords(name, description, topics) {
    const lowerName = name.toLowerCase();
    const lowerDesc = (description ?? "").toLowerCase();
    const lowerTopics = (topics ?? []).map((t) => t.toLowerCase());
    // Score each type
    const scores = {};
    for (const [repoType, keywords] of Object.entries(NAME_KEYWORDS)) {
 ...
 */
function isResourceRepo(name, fileList) {
    ...
    if (fileList && fileList.length > 0) {
        const codeFiles = fileList.filter((f) => f.type === "file" &&
            !f.name.endsWith(".md") &&
            !f.name.endsWith(".txt") &&
            f.name !== "LICENSE" &&
            f.name !== ".gitignore");
...
export function classifyRepo(input) {
    ...
    const isExperimental = !readmeContent || readmeContent.trim().length < MIN_README_LENGTH;
