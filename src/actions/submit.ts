...
export async function submitResource(
  ...
  try {
    ...
    if (
      isGitHubRepo &&
      (stats.stars === undefined || stats.forks === undefined)
    ) {
        ...
        try {
          ...
          if (details) {
            stats = {
              ...
              license:
                details.license?.spdx_id ?? details.license?.name ?? undefined,
            };
          }
        } catch (e) {
    ...
    // Parse builtWith from comma-separated string to {name, slug}[]
    const parsedBuiltWith = validatedData.builtWith
      ? validatedData.builtWith
          .split(",")
          .map((t) => {
            const name = t.trim();
