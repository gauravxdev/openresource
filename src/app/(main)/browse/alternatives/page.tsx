...
import { getAlternativeResources } from "@/actions/resources";
...
export default async function AlternativeResourcesPage({
  searchParams,
}: Props) {
  ...
  const {
    ...
    totalCount,
    success,
  } = await getAlternativeResources(page);
    ...
    <div className="mx-auto max-w-[1152px] px-4 py-8 md:px-6 md:py-12">
      <div className="mb-8 text-center md:mb-12 md:text-left">
        ...
        <p className="text-muted-foreground w-full text-lg md:max-w-3xl">
          Discover free and open-source alternatives to popular proprietary
      ...
      {!success || !resources || resources.length === 0 ? (
        <div className="border-border/60 bg-muted/10 flex flex-col items-center justify-center rounded-xl border border-dashed px-4 py-20 text-center">
          ...
          <h3 className="mb-2 text-xl font-semibold">No alternatives found</h3>
          <p className="text-muted-foreground max-w-sm">
            Check back later! We are constantly adding new open-source
            alternatives.
          </p>
        </div>
      ) : (
